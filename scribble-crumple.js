(()=>{
  ...
  style.textContent=`
    ...
    .scribble-trash-stage{position:relative;min-height:250px;margin:12px 0;background:linear-gradient(180deg,#f8f8f5,#eef2f0);border:1px solid var(--line,#d8e3df);border-radius:20px;overflow:hidden;display:flex;align-items:flex-start;justify-content:center;padding:18px}
    .scribble-trash-stage[hidden]{display:none!important}
    .scribble-paper{position:relative;width:88%;height:170px;background:#fffdf7;border:1px solid #e8dfcf;border-radius:5px;box-shadow:0 8px 20px rgba(55,50,40,.10);overflow:hidden;transform-origin:center center;z-index:2}
    .scribble-paper img{width:100%;height:100%;object-fit:contain;display:block}
    .scribble-bin{position:absolute;z-index:1;bottom:14px;width:92px;height:54px;background:#50656d;color:#fff;border-radius:10px 10px 17px 17px;text-align:center;font-size:25px;line-height:30px;box-shadow:inset 0 5px 0 rgba(255,255,255,.08)}
  ...
  status.textContent='描いたら、そのまま紙みたいに丸めて捨てられるよ。';
  ...
  canvas.insertAdjacentElement('afterend',stage);
  ...
  trashBtn.addEventListener('click',async()=>{
    ...
    paper.classList.add('is-crumpling');
    status.textContent='ぐしゃっと丸めて…';

    const paperAnim=paper.animate([
      {offset:0,transform:'translateY(0) rotate(0deg) scale(1)',width:'88%',height:'170px',borderRadius:'5px',opacity:1},
      {offset:.18,transform:'translateY(3px) rotate(-9deg) scale(.94)',width:'76%',height:'145px',borderRadius:'14px 28px 19px 31px',opacity:1},
      {offset:.38,transform:'translateY(22px) rotate(48deg) scale(.9)',width:'118px',height:'94px',borderRadius:'43% 57% 39% 61%',opacity:1},
      {offset:.56,transform:'translateY(50px) rotate(150deg) scale(.96)',width:'70px',height:'66px',borderRadius:'48% 52% 46% 54%',opacity:1},
      {offset:.78,transform:'translateY(116px) rotate(330deg) scale(.9)',width:'57px',height:'57px',borderRadius:'50%',opacity:1},
      {offset:1,transform:'translateY(176px) rotate(540deg) scale(.18)',width:'46px',height:'46px',borderRadius:'50%',opacity:0}
    ...
    setTimeout(()=>{
      paperAnim.cancel();binAnim.cancel();paper.classList.remove('is-crumpling');stage.hidden=true;canvas.style.visibility='visible';trashBtn.disabled=false;clearBtn.disabled=false;running=false;
    },260);
  });
})();